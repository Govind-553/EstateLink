...
function setupFirebaseAndForms() {
    ...
    if (loginForm) loginForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          window.location.href = "rentProperties.html";
        } else if (response.status === 403 && data.subscriptionActive === false) {
          // Subscription expired / not active – ask user to subscribe now
          showCustomPopup(
            data.message || "⚠️ Your subscription has expired. Please subscribe to continue.",
            "Subscription Required",
            () => {
              // Start Razorpay subscription flow as renewal
  ...
  const setupOtpLogic = (mode, phoneNumber) => {
    ...
    verifyOtpBtn.addEventListener('click', async (event) => {
      ...
      try {
        ...
        showCustomPopup(
          "✅ OTP verified. To start your account, please pay the subscription fee now. You will 1 month subscription.",
          "Subscription Required",
          async () => {
            ...
            try {
              ...
              if (data.status === 'success') {
                // Start subscription flow in "registration" mode
  ...
  const handleSubscription = async (mobileNumber, mode = "registration", loginPassword = null) => {
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.message || "⚠️ Failed to create subscription.");
      }
